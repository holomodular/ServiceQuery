...
namespace WebApp.Extensions
{
    public static class ServiceCollectionExtensions
    {
        public static IServiceCollection AddCustomWebsite(this IServiceCollection services, IConfiguration Configuration)
        {
            ...
            // Register Database InMemory
            var builder = new DbContextOptionsBuilder<InMemoryDatabaseContext>();
            builder.UseInMemoryDatabase(Guid.NewGuid().ToString(), b => b.EnableNullChecks(false));
            services.Configure<DbContextOptions<InMemoryDatabaseContext>>(o => { o = builder.Options; });
            services.AddSingleton<DbContextOptions<InMemoryDatabaseContext>>(builder.Options);
            services.AddDbContext<InMemoryDatabaseContext>(c => { c = builder; }, ServiceLifetime.Scoped);
