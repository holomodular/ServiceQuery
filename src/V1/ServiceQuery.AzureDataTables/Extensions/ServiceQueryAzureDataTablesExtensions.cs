...
namespace ServiceQuery
{
    ...
    public static class ServiceQueryAzureDataTablesExtensions
    {
        ...
        /// <param name="azureDataTablesOptions"></param>
        /// <returns></returns>
        public static async Task<ServiceQueryResponse<T>> ExecuteAsync<T>(this IServiceQuery serviceQuery, TableClient tableClient, ServiceQueryOptions serviceQueryOptions = null, AzureDataTablesOptions azureDataTablesOptions = null)
                    where T : class, ITableEntity
        {
            if (tableClient == null)
                return null;
            ...
            try
            {
                AsyncPageable<T> asyncpagableResult;
                int pageSize = serviceQuery.PageSize;
                if (pageSize > 1000)
                    pageSize = 1000;
                ...
                if (returnAllRecords)
                    whereExp = null;
                if (whereExp != null)
                    asyncpagableResult = tableClient.QueryAsync<T>(whereExp, pageSize, selectProperties);
                else
                    asyncpagableResult = tableClient.QueryAsync<T>(maxPerPage: pageSize, select: selectProperties);
                ...
                long endRecordCount = serviceQuery.PageNumber * serviceQuery.PageSize;
                List<T> allRecords = new List<T>();
                var enumerator = asyncpagableResult.GetAsyncEnumerator();
                ...
                if (returnAllRecords)
                {
                    ...
                    if (serviceQuery.Filters != null && serviceQuery.Filters.Where(x => x.FilterType == ServiceQueryFilterType.Aggregate).Any())
                    {
                        ...
                        {
                            response.List = new List<T>(); //don't return any records
                            response.Aggregate = await serviceQuery.ExecuteAggregateAsync<T>(allRecords.AsQueryable(), serviceQueryOptions);
                            return response;
                        }
                    }
                    if (serviceQuery.Filters != null && serviceQuery.Filters.Where(x => x.FilterType == ServiceQueryFilterType.Distinct).Any())
