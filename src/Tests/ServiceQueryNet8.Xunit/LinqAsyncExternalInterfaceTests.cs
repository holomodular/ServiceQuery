...
{
    public class LinqAsyncExternalInterfaceTests : BaseTest
    {
        ...
        public async Task ExternalInterfaceStandardTest()
        {
            ...
            result = await sourceQueryable.ServiceQueryExpand().Where(whereExp).ToAsyncEnumerable().ToListAsync();
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            ...
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            result = await sourceQueryable.ServiceQueryExpand().Where(whereExp).Select(selectExp).ToAsyncEnumerable().ToListAsync();
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            retRecord = result[0];
