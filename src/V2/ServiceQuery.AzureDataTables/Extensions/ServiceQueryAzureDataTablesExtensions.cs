            ...
            where T : class, ITableEntity
        {
            ...
            try
            {
                ...
                if (returnAllRecords)
                    whereExp = null;
                if (whereExp != null)
                    pagableResult = tableClient.QueryAsync<T>(whereExp, pageSize, selectProperties);
                else
                    pagableResult = tableClient.QueryAsync<T>(maxPerPage: pageSize, select: selectProperties);
                ...
                long endRecordCount = serviceQuery.PageNumber * serviceQuery.PageSize;
                List<T> allRecords = new List<T>();
                await foreach (var page in pagableResult.AsPages())
                {
                    pageCount++;
                    foreach (var item in page.Values)
                    {
