namespace ServiceQuery.Xunit
{
    public class PagingTests : BaseTest
    {
        ...
        public async Task NegativePagingRequestTest()
        {
            ...
            x.FilterType == ServiceQueryServiceFilterType.pagenumber).First().Values = new List<string>() { "a" };
            ...
            {
                result = await serviceQuery.ExecuteAsync<TestClass>(sourceQueryable);
            });

            serviceQuery = ServiceQueryRequestBuilder.New().Paging(0, 0, false).Build();
            serviceQuery.Filters.Where(x =>
            x.FilterType == ServiceQueryServiceFilterType.pagesize).First().Values = new List<string>() { "a" };
            Assert.Throws<ServiceQueryException>(() =>
            {
                result = serviceQuery.Execute(sourceQueryable);
            });
            ...
            serviceQuery = ServiceQueryRequestBuilder.New().Paging(0, 0, false).Build();
            serviceQuery.Filters.Where(x =>
            x.FilterType == ServiceQueryServiceFilterType.includecount).First().Values = new List<string>() { "a" };
            Assert.Throws<ServiceQueryException>(() =>
            {
                result = serviceQuery.Execute(sourceQueryable);
            });
        ...
        public async Task PagingRequestTest()
        {
            ...
            Assert.NotNull(result.List);
            Assert.True(result.List.Count == 0);
            result = await serviceQuery.GetServiceQuery().ExecuteAsync(sourceQueryable);
            Assert.NotNull(result);
            Assert.True(result.Count == 0);
            Assert.True(!result.Aggregate.HasValue);
            Assert.NotNull(result.List);
