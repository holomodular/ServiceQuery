...
{
    public class ExternalInterfaceTests : BaseTest
    {
        ...
        public async Task ExternalInterfaceStandardTest()
        {
            ...
            result = sourceQueryable.ServiceQueryExpand().Where(whereExp).ToList();
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            Assert.True(result[0].LongVal == 1);
            result = await sourceQueryable.ServiceQueryExpand().Where(whereExp).ToListAsync();
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            ...
            whereExp = serviceQuery.BuildWhereExpression<TestClass>();
            result = sourceQueryable.ServiceQueryExpand().Where(whereExp).Select(selectExp).ToList();
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            result = await sourceQueryable.ServiceQueryExpand().Where(whereExp).Select(selectExp).ToListAsync();
            Assert.NotNull(result);
            Assert.True(result.Count == 1);
            retRecord = result[0];
            oneRecord = new TestClass().GetDefault1Record();
        ...
        public async Task NullExternalInterfaceStandardTest()
        {
            ...
            Assert.NotNull(selectExp);
            whereExp = serviceQuery.BuildWhereExpression<TestClass>();
            result = sourceQueryable.ServiceQueryExpand().Where(whereExp).Select(selectExp).OrderByDescending(x => x.IntVal).ToList();
            Assert.NotNull(result);
            Assert.True(result.Count == 4);
            result = await sourceQueryable.ServiceQueryExpand().Where(whereExp).Select(selectExp).ToListAsync();
            Assert.NotNull(result);
            Assert.True(result.Count == 4);
            retRecord = result[0];
            oneRecord = new TestClass().GetDefault3Record();
        ...
        public async Task ExternalInterfaceRequestExecuteTest()
        {
            ...
            Assert.Null(result);
            result = await serviceQuery.ExecuteAsync(nullquery);
            Assert.Null(result);
            ...
            result = await serviceQuery.ExecuteAsync(sourceQueryable);
            Assert.NotNull(result);
            Assert.Null(result.Aggregate);
            Assert.True(result.Count == 0);
            Assert.NotNull(result.List);
